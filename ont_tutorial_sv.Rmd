...
```{r loadVcf, echo=FALSE, message=FALSE, warning=FALSE}
...
suppressWarnings(variants <- locateVariants(rowRanges(vcf), txdb, AllVariants()))
# filter on some key columns; remove e.g. the TXID and CDSID that lead to duplicates ...
variants <- data.frame(variants)[,c("seqnames", "start", "end", "width", "LOCATION", "QUERYID", "GENEID")]
...
kable(as.data.frame(table(as.character(variants$LOCATION))), format="html", col.names=c("SV location", "Count"), caption="Table summarising the genomic location of SVs called", booktabs=TRUE, table.envir='table*', linesep="") %>% kable_styling(c("striped", "condensed"))
```
...
geneAssociation <- cbind(genes, chromosome, insertions, deletions, duplications, vsizes)
geneAssociation[geneAssociation=="0"] <- " "


kable(geneAssociation, format="html", col.names=c("GeneId", "Chromosome", "Insertions", "Deletions", "Duplications", "Mean variant size"), caption="Table listing genes containing SVs, the SV type and SV size", booktabs=TRUE, table.envir='table*', linesep="") %>% kable_styling(c("striped", "condensed"))
```
...
```{r RepeatMasker, echo=FALSE, message=FALSE, warning=FALSE} 

RepeatGR <- RepeatMaskerGR()
SV <- Vcf2GRanges(vcfFile)
seqlengths(SV) <- getSeqLengths(names(seqlengths(SV)))

suppressWarnings(real <- RepeatGR[queryHits(findOverlaps(RepeatGR, SV))]$repeatClass)
realDF <- as.data.frame(table(real))


shuffleGRanges <- function(i, SV, RepeatGR) {
  sstarts <- as.numeric(round(runif(length(SV)) * seqlengths(SV)[match(as.character(seqnames(SV)), names(seqlengths(SV)))] - width(SV)))
  
  # this sampling is naive and reads could be assigned to N regions of the genome .... this can be tested ...
  
  shGR <- GRanges(seqnames=seqnames(SV),
                  ranges=IRanges(start=sstarts,
                                 end=sstarts+width(SV)-1))
  return(RepeatGR[queryHits(findOverlaps(RepeatGR, shGR))]$repeatClass)
}

replicates <- 101
suppressWarnings(simulated <- lapply(seq(replicates), shuffleGRanges, SV=SV, RepeatGR=RepeatGR))
simDF <- as.data.frame(table(unlist(simulated)) / replicates)

simulationTest <- merge(x=realDF, y=simDF, by=1, all=TRUE)
selectedRepeatsA <- simulationTest[which(as.numeric(simulationTest$Freq.x) > as.numeric(simulationTest$Freq.y)*2.5) ,]
selectedRepeatsA$scale <- selectedRepeatsA$Freq.x / selectedRepeatsA$Freq.y
selectedRepeatsB <- simulationTest[which(as.numeric(simulationTest$Freq.x)*2.5 < as.numeric(simulationTest$Freq.y)) ,]
selectedRepeatsB$scale <- selectedRepeatsB$Freq.y / selectedRepeatsB$Freq.x
selectedRepeats$Freq.y <- round(selectedRepeats$Freq.y, digits=1)

diffAbund <- rbind(selectedRepeatsA, selectedRepeatsB)
diffAbund <- diffAbund[order(diffAbund$scale, decreasing=TRUE),]
diffAbund$Freq.y <- round(diffAbund$Freq.y, digits=1)
diffAbund$scale <- round(diffAbund$scale, digits=1)

kable(diffAbund, format="html", col.names=c("repeat class", "Observed", "Expected", "X"), caption="Repeat classes overlapping with SVs and their over- or under-representation", booktabs=TRUE, table.envir='table*', linesep="") %>% kable_styling(c("striped", "condensed"))
```
