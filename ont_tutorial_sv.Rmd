...
```{r fastqSummary, cache=TRUE, echo=FALSE}
...
file.data <- t(as.data.frame(c(filename=basename(input_fastq), checksum=nanopoRe::md5sum(input_fastq), reads=scales::comma_format()(getFastqCount()), bases=bases, size=gdata::humanReadable(file.info(input_fastq)$size))))
...
```{r loadMapData, echo=FALSE, results="hide"}
...
# junk the non-canonical chromosome ids
chrIds <- getChromosomeIds()

# this is deprecated with new version of the nanopoRe package
# cleanReferenceGenome(chrIds[grep("(MT)|(\\.)", chrIds)])
...
totalSeqs <- getFastqCount()
primaryMaps <- nrow(primaryBAM)
unmapped <- nrow(unmappedBAM)

totalNucleotides <- getFastqBases()
...
plotDepthOfCoverageMegablock(coverageData, colMax=colMax)
