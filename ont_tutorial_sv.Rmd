...
IGV can also view deletions - the figure below shows what could be a 130 bp homozygous deletion. There is read support for this deletion and the deletion has been sampled from both DNA sequence strands.

![](Static/Images/SV_homozygous_deletion.PNG)

**`IGV`** can be started from the command line by using the command `igv`.

We would like to explore the read mapping information around the SV located within the **MUC7** gene. The command below will open the **`IGV`** browser to the appropriate genome coordinates - in this example we are using the feature with coordinates `[Chr 4] Start=1387837, Stop=1389955` - this corresponds to a region 1kb up- and down-stream on the annotated deletion within the **`CRIPAK`** gene - this should look much the heterozygous deletion figure shown above.


```
igv -g ./ReferenceData/human_g1k_v37_chr4.fasta ./Analysis/GM24385.nf7.chr4_a/alignment/GM24385.nf7.chr4_a_minimap2.bam 4:1387837-1389955
```
