...
The **`ont_tutorial_sv`** script is used to generate this report. The configuration file, **`config.yaml`** has been parsed and the fastq format DNA sequence file has been identified as **` `r input_fastq` `**.
...
The table above provides an abbreviated summary of the provided fastq format sequence file used by the tutorial. The **md5sum** is a checksum that can be used to track the integrity of a specified sequence file - this value should be unique to each DNA sequence file.
...
The table above summarises the sequence mapping characteristics for each of the chromosomes specified within the provided reference genome. 
...
infographicFile3 <- infoGraphicPlot3(identifier="truthset_validation", 
                 panelA=c(key="Precision",   value="0.964", icon="fa-dot-circle-o"), 
                 panelB=c(key="Recall",    value="0.929", icon="fa-braille"), 
                 panelC=c(key="F1 score", value="94.6", icon="fa-percent"))
